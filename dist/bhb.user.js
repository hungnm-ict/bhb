...
(() => {
  ...
  var rates = { real: 0, game: 0, effective: 1 };
  ...
  var BUDGET_MIN_MS = 3;
  var BUDGET_MAX_MS = 25;
  var BUDGET_STEP_MS = 2;
  var FPS_FLOOR = 25;
  var FPS_CEILING = 45;
  var frameBudget = 15;
  ...
  function reportFrameRate(measured) {
  ...
  function createVirtualClock(readReal) {
    ...
    read2.drift = () => virtual === null ? 0 : virtual - readReal();
    read2.reset = () => {
      virtual = readReal();
      previous = virtual;
    };
  ...
  function sortToDefaultOrder(activities) {
  ...
  function createEngine(deps) {
    ...
    function runSequence(steps, canvas, gl, screenId) {
      ...
      for (let at = cursor.index % steps.length; at < steps.length; at += 1) {
        ...
        const isBlocking = expected.kind === StepKind.WAIT || expected.kind === StepKind.COUNT;
    ...
    function schedulePoll() {
      ...
      const delay = resting ? SCRIPT_PACE_LADDER[SCRIPT_PACE_LADDER.length - 1] : isCounting ? COUNT_POLL_MS : pace;
  ...
  function createStepEditor(deps) {
    ...
    function duplicate(stepId) {
      ...
      const original2 = steps[index];
      const copy = {
        ...original2,
        id: createStepId(),
        // Places of its own: nudging the copy must not move the original.
        points: original2.points.map((point) => ({ ...point })),
        screens: [...original2.screens || []],
        ...
        label: isAutoLabel(original2.label) ? original2.label : `${original2.label || ""} (2)`.trim()
      };
    ...
    function move(stepId, delta, shownIds) {
      const steps = deps.getSteps();
      const shown = Array.isArray(shownIds) && shownIds.length > 0 ? shownIds : steps.map((step2) => step2.id);
      const among = shown.indexOf(stepId);
      const neighbour = shown[among + delta];
      if (among === -1 || neighbour === void 0) {
        return;
      }
      const from = steps.findIndex((step2) => step2.id === stepId);
      const to = steps.findIndex((step2) => step2.id === neighbour);
      if (from === -1 || to === -1) {
        return;
      }
...
`;
  ...
  var LOG_LIMIT = 200;
  function resolveStepFilter(chosen, runTarget, activities = []) {
    if (chosen !== void 0) {
      return chosen;
    }
    if (runTarget === "script") {
  ...
  function cloneStepsInto(all, source, activityId) {
  ...
  var peekWatchers = /* @__PURE__ */ new WeakSet();
  ...
  function renderStepsTab(deps) {
    ...
    const stepRows = steps.map((step, index) => {
      ...
      const liveRow = live ? el("div", { class: "bhb-live" }, [
  ...
  function renderScreensTab(deps) {
    ...
    const clash = matching.length > 1 ? el("p", { class: "bhb-note bhb-note--warn bhb-screens__clash" }, [
      el("span", {
  ...
  function renderCanvasLock(deps, toggleRow) {
    ...
    return el("div", { class: "bhb-field" }, [
      ...
      lock.enabled ? null : el("p", { class: "bhb-note bhb-note--warn", text: t("lock.offWarning") }),
  ...
  var OWN_SURFACES = ".bhb-panel, .bhb-probes, .bhb-drag, .bhb-hud";
  function shieldOwnFields() {
  ...
  function bootstrap() {
    ...
    const stepEditor = createStepEditor({
      ...
      getCaptureActivity: () => resolveStepFilter(store.get().stepFilter, settings.runTarget, getActivities()) || null,
    ...
    const panel = createPanel({
      ...
      getCanvasLock: () => settings.canvasLock.enabled ? { width: settings.canvasLock.width, height: settings.canvasLock.height } : null,
